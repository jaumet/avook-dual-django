function loadCatalog(base){
  console.log("Loading catalog with base data:", base);
  ...
  for (const title in base) {
    ...
    `;
    card.onclick = () => {
      console.log(`Clicked on title: ${i.id}`);
    };
...
function initFilters(base){
  console.log("Initializing filters with base data:", base);
  ...
  console.log("Available levels:", [...levSet]);
...
if (titlesDataElement) {
    console.log("Found titles-data element.");
    ...
    console.log("Transformation complete. Calling loadCatalog.");
    loadCatalog(base);
} else {
    console.error("Could not find titles-data element.");
}
